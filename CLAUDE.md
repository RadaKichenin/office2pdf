...
- For visual bug fixes tied to an issue, keep `assets/bugfixes/issue-<number>/gt.jpg`, `before.jpg`, `after.jpg`, and `layout-audit.json` generated from the same fixture and output; every fix PR must change `after.jpg` and `layout-audit.json`. Generate the report with `compare_layout.py --json --audit`; classify page-count, missing/extra/reflow or changed-wrap, and large-shift failures with open issues in both the report fields and `Remaining:` rows. Use progressive JPEG quality 86 with metadata stripped, preserve the source pixel dimensions, and verify text and images remain legible for direct GitHub links. Strip first, then re-set the density (`-strip -density 150 -units PixelsPerInch`): the contract check rejects evidence recording under 150 DPI, which stripping alone removes.
